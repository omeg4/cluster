...
SGSIndSuite[pars_, ezrange_, drange_] := Module[
  {make,
   normed,
   processed,
   ei = ezrange[[1]],
  ...
  make = {
    Table[{dd, 
      Table[{Eperp, 
        IndCoul[2, pars[[2 ;;]], pars[[1]], Eperp, 4.89, 
  ...
  Table[
	  Export[ToString@StringForm["calcs_d`1`_e`2`.m",dn,en],{
		  {"min",normed[[1]][[dn]][[en]]},
		  {"max",normed[[2]][[dn]][[en]]}
	  }],
	  {dn, 1 +(df-di)/dstep}, {en, 1 + (ef-ei)/estep}
  ];
]
